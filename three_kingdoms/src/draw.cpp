	...
	uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    std::cout << "fill " << area << ' ' << color_t{r,g,b,a}<<std::endl;
...
void draw::fill(uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    std::cout << "fill " << color_t{r,g,b,a}<<std::endl;
	SDL_SetRenderDrawColor(renderer(), r, g, b, a);
...
void draw::set_color(uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    std::cout << "set color " << color_t{r,g,b,a}<<std::endl;;
	SDL_SetRenderDrawColor(renderer(), r, g, b, a);
}
...
void draw::set_color(uint8_t r, uint8_t g, uint8_t b)
{
    ...
    std::cout << "set color " << color_t{r,g,b}<<std::endl;
	...
	uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
    ...
    std::cout << "rect " << rect << ' ' << color_t{r,g,b,a}<<std::endl;
	SDL_SetRenderDrawColor(renderer(), r, g, b, a);
	if (sdl_bad_at_rects()) {
		return draw_rect_as_lines(rect);
	}
	...
	bool flip_v)
{
    ...
    { 
        // DBG_D << "null flipped"; 
        std::cout << "null flipped"<<std::endl;
        return; 
    }
	...
	bool centered, bool mirrored)
{
	if (!tex) 
    { 
        // DBG_D << "null tiled_highres"; 
        std::cout << "null tiled_highres"<<std::endl;
        return; 
    }
	...
	const std::array<SDL_Vertex, 4>& verts)
{
    ...
    std::cout << "smooth shade, verts:"<<std::endl;
...
void draw::force_clip(const SDL_Rect& clip)
{
	...
	if (!renderer()) {
        ...
        std::cout << "trying to force clip will null renderer"<<std::endl;
		return;
	}
	// DBG_D << "forcing clip to " << clip;
    std::cout << "forcing clip to " << clip << std::endl;
...
void draw::force_viewport(const SDL_Rect& viewport)
{
	if (!renderer()) {
        ...
        std::cout << "trying to force viewport will null renderer"<<std::endl;
		return;
	}
	// DBG_D << "forcing viewport to " << viewport;
    std::cout << "forcing viewport to " << viewport << std::endl;
...
SDL_Rect draw::get_viewport()
{
	if (!renderer()) {
        ...
        std::cout << "no renderer available to get viewport" << std::endl;
		return sdl::empty_rect;
	}
	...
	, viewport_()
{
	...
	if (!renderer()) {
        ...
        std::cout << "can't set render target with null renderer"<<std::endl;
		return;
	}
...
draw::render_target_setter::~render_target_setter()
{
	if (!renderer()) {
        ...
        std::cout << "can't reset render target with null renderer"<<std::endl;
		return;
	}
...
draw::render_target_setter draw::set_render_target(const texture& t)
{
	if (t) {
		...
		//       << t.w() << 'x' << t.h() << " texture";
        std::cout << "setting render target to "
		      << t.w() << 'x' << t.h() << " texture"<<std::endl;
	} else {
		// DBG_D << "setting render target to main render buffer";
        std::cout << "setting render target to main render buffer"<<std::endl;
	}
