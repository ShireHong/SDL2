...
static uint32_t timer_callback(uint32_t, void* id)
{
    ...
    std::cout << "Pushing timer event in queue.";
				 ...
				 const bool repeat)
{
	...
	if(timer.sdl_id == 0) {
        ...
        std::cout << "Failed to create an sdl timer."<<std::endl;
		return 0;
	}
    ...
    std::cout << "Added timer " << next_timer_id << "."<<std::endl;
...
bool remove_timer(const std::size_t id)
{
    ...
    std::cout << "Removing timer " << id << "."<<std::endl;
	// std::scoped_lock lock(timers_mutex);

	auto itor = get_timers().find(id);
	if(itor == get_timers().end()) {
		// LOG_GUI_E << "Can't remove timer since it no longer exists.";
        std::cout << "Can't remove timer since it no longer exists."<<std::endl;
		return false;
	}
	...
	if(!SDL_RemoveTimer(itor->second.sdl_id)) {
        ...
        std::cout << "The timer is already out of the SDL timer list."<<std::endl;
...
bool execute_timer(const std::size_t id)
{
    ...
    std::cout << "Executing timer " << id << "."<<std::endl;
	std::function<void(size_t)> callback = nullptr;
	{
		...
		if(itor == get_timers().end()) {
            ...
            std::cout << "Can't execute timer since it no longer exists."<<std::endl;
			return false;
		}
