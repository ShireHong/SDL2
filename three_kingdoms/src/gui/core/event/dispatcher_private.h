	...
	F&&... params)
{
	...
	// {
	for (auto it = event_chain.rbegin(); it != event_chain.rend(); ++it) 
    {
        const auto& chain_target = it->first;
        const auto& chain_event = it->second;
    ...
    // {
    for (auto it = event_chain.begin(); it != event_chain.end(); ++it) 
    {
        const auto& chain_target = it->first;
        const auto& chain_event = it->second;
		const auto& signal = dispatcher_implementation::event_signal<C>(*chain_target, chain_event);
