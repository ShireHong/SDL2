...
void sdl_event_handler::disconnect(dispatcher* disp)
{
	...
	std::cout << "removing dispatcher " << static_cast<void*>(disp)<<std::endl;
	/***** Remove dispatcher. *****/
	...
	if(dispatchers_.empty()) {
		...
		std::cout << "deleting unused dispatcher event context"<<std::endl;
		leave();
...
void sdl_event_handler::video_resize(const point& new_size)
{
	...
	std::cout << "Firing: " << SDL_VIDEO_RESIZE << "."<<std::endl;
...
void sdl_event_handler::raw_event(const SDL_Event& event) {
	...
	std::cout << "Firing raw event" << std::endl;
...
void sdl_event_handler::mouse(const ui_event event, const point& position)
{
	...
	// 	}
	// }
	for(auto it = dispatchers_.rbegin(); it !=  dispatchers_.rend(); ++it)
	{
		auto &dispatcher = *it;
		if(dispatcher->get_mouse_behavior() == dispatcher::mouse_behavior::all) {
			dispatcher->fire(event, dynamic_cast<widget&>(*dispatcher), position);
						...
						const std::string& unicode)
{
	...
	std::cout << "Firing: " << SDL_KEY_DOWN << "."<<std::endl;
	if(dispatcher* dispatcher = keyboard_dispatcher()) {
...
void sdl_event_handler::close_window(const unsigned window_id)
{
	...
	std::cout << "Firing " << CLOSE_WINDOW << "."<<std::endl;
