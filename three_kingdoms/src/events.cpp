	...
	, has_joined_global_(that.has_joined_global_)
{
	if(has_joined_global_) {
		...
		// 	}
		// }
		for(auto it = event_contexts.rbegin(); it != event_contexts.rend(); ++it) 
		{
			auto &context = *it;
			if(context.has_handler(&that)) 
			{
	...
	for(SDL_Event& event : events) {
		...
		case SDL_WINDOWEVENT:
			...
			case SDL_WINDOWEVENT_EXPOSED:
				...
				std::cout << "events/EXPOSED"<<std::endl;
		...
		if(event_contexts.empty() == false) {
			...
			for(auto it = h.begin(); it != h.end(); ++it) {
				...
				if(event_contexts.size() != ec_index) {
					...
					std::cout << "ec size changed! bugging out"<<std::endl;
					break;
				}
				if(h_size != h.size()) {
					// LOG_EV << "h size changed! bugging out";
					std::cout << "h size changed! bugging out"<<std::endl;
					break;
				}
			}
		}
