...
void render_screen()
{
	...
	if(SDL_GetRenderTarget(*window) != render_texture_) {
		std::cout << "trying to render screen, but current render texture is "
			<< static_cast<void*>(SDL_GetRenderTarget(*window))
			<< " | " << static_cast<void*>(current_render_target_.get())
			<< ". It should be " << static_cast<void*>(render_texture_.get())<<std::endl;
...
void update_buffers(bool autoupdate)
{
	...
	std::cout << "updating video buffers"<<std::endl;
