...
public:
	...
	/** See @ref widget::find. */
	widget* find(const std::string& id, const bool must_be_active) ;
...
private:
	...
	virtual const std::string& get_control_type() const ;
...
struct window_definition : public styled_widget_definition
{
	...
	struct resolution : public panel_definition::resolution
	{
        ...
        explicit resolution(const int win_w, 
                            ...
                            const int max_w,
                            const int max_h);
		explicit resolution(const resolution_definition& resol_def);
