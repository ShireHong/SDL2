...
#include "gui/widgets/grid_private.h"
					  ...
					  const unsigned border_size)
{
	...
	if(cell.get_widget()) {
		...
		std::cout << LOG_HEADER << " child '" << cell.id() << "' at cell '"
				  << row << ',' << col << "' will be replaced." << std::endl;
	}
...
void grid::reduce_width(const unsigned maximum_width)
{
	...
	std::cout << LOG_HEADER << " maximum width " << maximum_width << "." << std::endl;
	point size = get_best_size();
	if(size.x <= static_cast<int>(maximum_width)) {
		// DBG_GUI_L << LOG_HEADER << " Already fits.";
		std::cout << LOG_HEADER << " Already fits." << std::endl;
		return;
	}
	...
	if(size.x <= static_cast<int>(maximum_width)) {
		...
		std::cout << LOG_HEADER << " Resize request honored." << std::endl;
		return;
	}
	...
	std::cout << LOG_HEADER << " Resizing failed." << std::endl;
...
void grid::request_reduce_width(const unsigned maximum_width)
{
	...
	for(std::size_t col = 0; col < cols_; ++col) {
		if(too_wide - reduced >= col_width_[col]) {
			...
			std::cout << LOG_HEADER << " column " << col
					  << " is too small to be reduced." << std::endl;
			continue;
		}
		...
		if(width < col_width_[col]) {
			...
			std::cout << LOG_HEADER << " reduced " << reduction
					  << " pixels for column " << col << "." << std::endl;
			size.x -= reduction;
			reduced += reduction;
		}
...
void grid::reduce_height(const unsigned maximum_height)
{
	...
	std::cout << LOG_HEADER << " maximum height " << maximum_height << "." << std::endl;
	point size = get_best_size();
...
void grid::request_reduce_height(const unsigned maximum_height)
{
	...
	for(std::size_t row = 0; row < rows_; ++row) {
		...
		if(too_high - reduced >= row_height_[row]) {
			...
			std::cout << LOG_HEADER << " row " << row << " height "
					  << row_height_[row] << " want to reduce " << too_high
					  << " is too small to be reduced fully try 1 pixel." << std::endl;
			wanted_height = 1;
		}
		...
		if(height < row_height_[row]) {
			...
			std::cout << LOG_HEADER << " row " << row << " height "
					  << row_height_[row] << " want to reduce " << too_high
					  << " reduced " << reduction << " pixels." << std::endl;
			size.y -= reduction;
			reduced += reduction;
		}
	...
	std::cout << LOG_HEADER << " Requested maximum " << maximum_height
			  << " resulting height " << size.y << "." << std::endl;
...
point grid::calculate_best_size() const
{
	...
	for(unsigned row = 0; row < rows_; ++row) {
		...
		std::cout << LOG_HEADER << " the row_height_ for row " << row
				  << " will be " << row_height_[row] << "." << std::endl;
	}
...
void grid::place(const point& origin, const point& size)
{
	...
	if(best_size.x > size.x || best_size.y > size.y) {
		...
		std::cout << LOG_HEADER << out.str() << "." << std::endl;
...
void grid::child::place(point origin, point size)
{
	...
	// 		  << " x " << widget_size << ".";
	std::cout << LOG_CHILD_HEADER << " resize widget to " << widget_orig
			  << " x " << widget_size << "." << std::endl;
		...
		grid& grid, const unsigned column, const unsigned maximum_width)
{
	...
	std::cout << LOG_IMPL_HEADER << " maximum column width " << maximum_width
			  << " returning " << required_width << "." << std::endl;
