...
point panel::border_space() const
{
	...
	point test =  point(conf->left_border + conf->right_border, conf->top_border + conf->bottom_border);
...
panel_definition::resolution::resolution(const resolution_definition& resol_def)
	: resolution_definition(resol_def)
	, top_border(10)
	, bottom_border(20)
	, left_border(15)
	, right_border(30)
{
	std::cout<<"panel_definition build"<<std::endl;
	// The panel needs to know the order.
	// state.emplace_back(VALIDATE_WML_CHILD(cfg, "background", missing_mandatory_wml_tag("panel_definition][resolution", "background")));
	// state.emplace_back(VALIDATE_WML_CHILD(cfg, "foreground", missing_mandatory_wml_tag("panel_definition][resolution", "foreground")));
}
