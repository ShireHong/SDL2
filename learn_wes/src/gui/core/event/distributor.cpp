...
void mouse_motion::signal_handler_sdl_mouse_motion(const event::ui_event event, bool& handled, const point& coordinate)
{
	...
	if(mouse_captured_) {
		...
		if(mouse_over) {
			...
			std::cout << LOG_HEADER << "Firing: " << event << "."<<std::endl;
...
void mouse_motion::mouse_enter(widget* mouse_over)
{
	...
	std::cout << LOG_HEADER << "Firing: " << event::MOUSE_ENTER << "."<<std::endl;
	assert(mouse_over);
...
void mouse_motion::mouse_hover(widget* mouse_over, const point& coordinate)
{
	...
	std::cout << LOG_HEADER << "Firing: " << event::MOUSE_MOTION << "."<<std::endl;
	assert(mouse_over);
...
void mouse_motion::show_tooltip()
{
	...
	std::cout << LOG_HEADER << "Firing: " << event::SHOW_TOOLTIP << "."<<std::endl;
	if(!hover_widget_) {
		// See mouse_motion::stop_hover_timer.
		// ERR_GUI_E << LOG_HEADER << event::SHOW_TOOLTIP << " bailing out, no hover widget.";
		std::cout << LOG_HEADER << event::SHOW_TOOLTIP << " bailing out, no hover widget."<<std::endl;
		return;
	}
...
void mouse_motion::mouse_leave()
{
	...
	std::cout << LOG_HEADER << "Firing: " << event::MOUSE_LEAVE << "."<<std::endl;
...
void mouse_motion::start_hover_timer(widget* widget, const point& coordinate)
{
	...
	if(hover_timer_) {
		...
		std::cout << LOG_HEADER << "Failed to add hover timer."<<std::endl;
	}
}
...
void mouse_motion::stop_hover_timer()
{
	if(hover_timer_) {
		...
		if(!remove_timer(hover_timer_)) {
			...
			std::cout << LOG_HEADER << "Failed to remove hover timer."<<std::endl;
		}
	...
	const event::ui_event event, bool& handled, const point& coordinate)
{
	...
	if(mouse_captured_) {
		...
		std::cout << LOG_HEADER << "Firing: " << mouse_data[I].sdl_button_down_event << "."<<std::endl;
		if(!owner_.fire(mouse_data[I].sdl_button_down_event, *focus_, coordinate)) {
			...
			std::cout << LOG_HEADER << "Firing: " << mouse_data[I].button_down_event << "."<<std::endl;
	...
	const event::ui_event event, bool& handled, const point& coordinate)
{
	...
	if(focus_) {
		...
		std::cout << LOG_HEADER << "Firing: " << mouse_data[I].sdl_button_up_event << "."<<std::endl;
		if(!owner_.fire(mouse_data[I].sdl_button_up_event, *focus_, coordinate)) {
			...
			std::cout << LOG_HEADER << "Firing: " << mouse_data[I].button_up_event << "."<<std::endl;
...
void mouse_button<I>::mouse_button_click(widget* widget)
{
	...
	if(last_click_stamp_ + settings::double_click_time >= stamp && last_clicked_widget_ == widget) {
		...
		std::cout << LOG_HEADER << "Firing: " << mouse_data[I].button_double_click_event << "."<<std::endl;
		...
		std::cout << LOG_HEADER << "Firing: " << mouse_data[I].button_click_event << "."<<std::endl;;
...
void distributor::keyboard_capture(widget* widget)
{
	if(keyboard_focus_) {
		...
		std::cout << LOG_HEADER << "Firing: " << event::LOSE_KEYBOARD_FOCUS << "."<<std::endl;
	...
	if(keyboard_focus_) {
		...
		std::cout << LOG_HEADER << "Firing: " << event::RECEIVE_KEYBOARD_FOCUS << "."<<std::endl;
...
void distributor::signal_handler_keyboard_internal(event::ui_event evt, P1&& p1, P2&& p2, P3&& p3)
{
	...
	std::cout << LOG_HEADER << evt << "."<<std::endl;
	if(keyboard_focus_) {
		...
		if(!control || control->get_active()) {
			...
			std::cout << LOG_HEADER << "Firing: " << evt << "."<<std::endl;
